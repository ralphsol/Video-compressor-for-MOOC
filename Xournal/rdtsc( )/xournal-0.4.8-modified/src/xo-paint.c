...
//int resmil;
unsigned long long tick_start, tick_end, tick_local;
int tick_base;

/*rdtsc wrapper function*/
...
void subdivide_cur_path(null)
{
  ...
  if(tick_base==0){
    ...
    sleep(1);
    tick_end = rdtsc();
    tick_local = (tick_end - tick_start);
    tick_base = 1;
    //printf("%llu\n", tick_local);
  }
...
void create_new_stroke(GdkEvent *event)
{
  ...
  tempVal = malloc(0*sizeof(*tempVal));
  tempVal = (int *) realloc(tempVal, 2*sizeof(*tempVal));
  tempVal[1] = 0;
  //ui.cur_item->timeVal = 0.000;
  //prevTime = clock();
  //clock_gettime(CLOCK_REALTIME, &start);

  //calculate tick_time
  //unsigned long long tick_start, tick_end;
...
void continue_stroke(GdkEvent *event)
{
  ...
  //*tempVal = realloc(*tempVal, ((1+ui.cur_path.num_points) * sizeof(float)));
  tempVal = (int *) realloc(tempVal, (1+ui.cur_path.num_points)*sizeof(*tempVal));
  ...
  //resmil = diff/(10E5);
  tempVal[ui.cur_path.num_points] = (int)((float)(tick_end-tick_start)*1000/(float)(tick_local));
  //ui.cur_item->timeVal = ((float)(clock() - prevTime))/CLOCKS_PER_SEC;
  //prevTime = clock();
  tick_start = tick_end;
